...
type ExtractTypeguard<T> = T extends (v: unknown, o?: TOpts) => v is infer U ? U : never;
export type TypeOf<A extends Schema<unknown>> = ExtractTypeguard<A>;

type OptionalSchemaKeys<TSchemaMap> = {
    [Index in keyof TSchemaMap]: Schema<undefined> extends TSchemaMap[Index] ? Index : never;
}[keyof TSchemaMap];
type NonOptionalSchemaKeys<TSchemaMap> = {
    [Index in keyof TSchemaMap]: Schema<undefined> extends TSchemaMap[Index] ? never : Index;
}[keyof TSchemaMap];
export type UnwrapSchemaMap<TSchemaMap> = keyof TSchemaMap extends never
    ? Record<string | number | symbol, undefined>
    : {
          [SchemaMapIndex in OptionalSchemaKeys<TSchemaMap>]?: TSchemaMap[SchemaMapIndex] extends Schema<unknown> ? TypeOf<TSchemaMap[SchemaMapIndex]> : never;
      } & {
...
type NonGenericExceptBooleans<A> = true extends Extends<string, A> | Extends<number, A> ? never : A;
type NonGeneric<A> = true extends Extends<boolean, A> ? never : NonGenericExceptBooleans<A>;
...
export function literal<TInner>(inner: NonGeneric<TInner>): (value: unknown) => value is NonGeneric<TInner> {
...
>(
    ...
    return (
        ...
        Object.keys(schema).every((key: string) => {
            const childMatches = schema[key] && schema[key]((value as Record<string, unknown>)[key], opts);
            if (!childMatches && opts?.logFailures) {
                console.log(`Member of shape ${JSON.stringify((value as Record<string, unknown>)[key])} for ${key} does not match expected type`);
            }
            return childMatches;
        })
    );
...
type TaggedUnionDefn<TTag extends string, TDefnSchemas extends TDefnSchemasForTags<TTag>> = {
    [TagValueLiteral in keyof TDefnSchemas]: FixOptionalIndices<{
        [ShapeItemKey in keyof TDefnSchemas[TagValueLiteral]]: TypeOf<TDefnSchemas[TagValueLiteral][ShapeItemKey]>;
    }>;
}[keyof TDefnSchemas];

export function taggedUnion<TTag extends string, TDefnSchemas extends TDefnSchemasForTags<TTag>>(tag: TTag, schemas: TDefnSchemas) {
    return (value: unknown, opts?: TOpts): value is TaggedUnionDefn<TTag, TDefnSchemas> => {
...
export function union<TLeft, TRight>(left: Schema<TLeft>, right: Schema<TRight>) {
...
export function intersection<TLeft, TRight>(left: Schema<TLeft>, right: Schema<TRight>) {
    return (value: unknown, opts?: TOpts): value is TLeft & TRight => {
        const matches = left(value, opts) && right(value, opts);
        if (!matches && opts?.logFailures) {
...
type TPluralIntersectionType<T> = (T extends Schema<infer U> ? (k: U) => void : never) extends (k: infer I) => void ? I : never;

export function intersectMany<TSchema extends Schema<unknown>>(inner: TSchema[]) {
    return (value: unknown, opts?: TOpts): value is TPluralIntersectionType<TSchema> => {
...
): (value: unknown, opts?: TOpts | undefined) => value is NonGenericExceptBooleans<TLiterals> {
    return unionMany(
        inners.map((value) => {
...
export function optional<TInner>(inner: Schema<TInner>): (value: unknown, opts?: TOpts | undefined) => value is TInner | undefined {
    return union(inner, undefinedtype);
}

export function nullable<TInner>(inner: Schema<TInner>): (value: unknown, opts?: TOpts | undefined) => value is TInner | null {
